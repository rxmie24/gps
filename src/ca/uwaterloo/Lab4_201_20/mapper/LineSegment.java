...
public class LineSegment {
	...
	private boolean isPointInSegment(float x, float y) {
		return ((x >= Math.min(start.x, end.x) && x <= Math.max(start.x, end.x)) || VectorUtils.isZero(x - start.x) || VectorUtils
				.isZero(x - end.x))
				&& ((y >= Math.min(start.y, end.y) && y <= Math.max(start.y, end.y)) || VectorUtils.isZero(y - start.y) || VectorUtils
						.isZero(y - end.y));
	}
	...
	public boolean containsPoint(PointF point) {
		if (!VectorUtils.areEqual(point.x * m + b, point.y))
			return false;
		// we know the point is on the line; now, if it is in the square denoted
		// by the start and end point, then it is in the segment
		return point.x < Math.max(start.x, end.x)
				&& point.x > Math.min(start.x, end.x)
				&& point.y < Math.max(start.y, end.y)
				&& point.y > Math.min(start.y, end.y)
				|| ((VectorUtils.areEqual(point.x, start.x) || VectorUtils.areEqual(point.x, end.x)) && (VectorUtils.areEqual(
						point.y, start.y) || VectorUtils.areEqual(point.y, end.y)));
	}

	/**
	 * Calculates a unit vector parallel to this line segment, with the same
	 * direction.
	 * 
	...
	/**
	 * Returns whether the other line segment is the same as this one, modulo
	 * direction. It would be inappropriate to override <code>.equals()</code>
	 * because we ignore direction.
	 * 
	 * @param other
	 *            the other line segment.
	...
	public boolean theSame(LineSegment other) {
		...
		if (!(VectorUtils.areEqual(start, other.start) && VectorUtils.areEqual(end, other.end) || VectorUtils.areEqual(start,
				other.end) && VectorUtils.areEqual(end, other.start))) {
			return false;
		}
	...
	/**
	 * Returns true iff the line segments overlap; weaker than
	 * <code>theSame</code>.
	 * 
	 * @param other
