...
package ca.uwaterloo.Lab4_201_20.mapper;
...
public class MapView extends View {
	...
	private PointF startPoint = new PointF(), destPoint = new PointF(), userPoint = new PointF(), selectPoint = new PointF();

	private List<PointF> userPath = new ArrayList<PointF>();
	private List<PointF> updatedUserPath = new ArrayList<PointF>();
	 ...
	 *            The points' location (in meters)
	 */
	public void setUpdatedUserPath(List<PointF> points) {
		updatedUserPath.clear();

		if (points != null)
			updatedUserPath.addAll(points);

		invalidate();
	}

	public List<PointF> getUpdatedUserPath() {
		return userPath;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (List<PointF> path : map.getPaths()) {
			for (int i = 0; i < path.size() - 1; i++) {
				canvas.drawLine(path.get(i).x * scale.x, path.get(i).y * scale.y, path.get(i + 1).x * scale.x,
						path.get(i + 1).y * scale.y, linePaints.get(LINE_COLOR_INDEX));
			}
		}

		for (int i = 0; i < userPath.size() - 1; i++) {
			canvas.drawLine(userPath.get(i).x * scale.x, userPath.get(i).y * scale.y, userPath.get(i + 1).x * scale.x,
					userPath.get(i + 1).y * scale.y, linePaints.get(USER_PATH_COLOR_INDEX));
		}

		for (int i = 0; i < updatedUserPath.size() - 1; i++) {
			canvas.drawLine(updatedUserPath.get(i).x * scale.x, updatedUserPath.get(i).y * scale.y,
					updatedUserPath.get(i + 1).x * scale.x, updatedUserPath.get(i + 1).y * scale.y,
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		menu.add(Menu.NONE, SET_LOCATION_ID, Menu.NONE, "Set as origin/current location");
		menu.add(Menu.NONE, SET_DESTINATION_ID, Menu.NONE, "Set as destination");
	}
